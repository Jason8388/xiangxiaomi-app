  ...
  if (!file) {
    return (
      <Screen>
        <PageHeader title="文件详情" />
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          <Text style={{ fontSize: 14, color: '#636E72' }}>文件不存在</Text>
        </View>
      </Screen>
    );
  }
      ...
      >
        ...
        >
          <View style={{ alignItems: 'center', marginBottom: 20 }}>
            ...
            <Text style={styles.fileNameLarge} numberOfLines={1}>
              {file.original_name}
            </Text>
        ...
        >
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>{file.uploader_name || '-'}</Text>
          </View>
          ...
          {file.description && (
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoValue}>{file.description}</Text>
            </View>
          )}
        </View>
        ...
        >
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <Text style={styles.sectionTitle}>标签 ({fileTags.length}/10)</Text>
          ...
          {fileTags.length > 0 ? (
            <View style={styles.tagsContainer}>
              {fileTags.map((tag: any) => (
                <View
                  key={tag.id}
                  style={[styles.tagItem, { borderColor: tag.color, backgroundColor: `${tag.color}10` }]}
                >
          ...
          {/* 可添加的标签 */}
          {availableTags.length > 0 && fileTags.length < 10 && (
          ...
          {fileTags.length >= 10 && (
            <Text style={styles.limitText}>已达到标签上限（10个）</Text>
          )}
        </View>
      </ScrollView>
              ...
              <ScrollView style={styles.modalBody}>
                ...
                <View style={{ marginBottom: 16 }}>
                  ...
                  <View style={styles.colorPicker}>
                    {['#1E88E5', '#00B894', '#FDCB6E', '#FF6B6B', '#6C63FF', '#00CEC9'].map((color) => (
                      <TouchableOpacity
                        key={color}
