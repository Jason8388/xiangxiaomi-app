...
router.get('/export', async (req, res) => {
  try {
    ...
    if (USE_DATABASE) {
      try {
        let query = 'SELECT * FROM customers ORDER BY id';
        let params: any[] = [];
        
        if (ids) {
          const idArray = (ids as string).split(',').map(id => parseInt(id.trim()));
          query = 'SELECT * FROM customers WHERE id = ANY($1) ORDER BY id';
          params = [idArray];
        }
        ...
        console.error('Database export error, falling back to memory:', dbError);
        // 如果数据库失败，使用内存存储
        customers = memoryCustomers;
        if (ids) {
          const idArray = (ids as string).split(',').map(id => parseInt(id.trim()));
          customers = memoryCustomers.filter((c: any) => idArray.includes(c.id));
        }
      }
    } else {
      // 使用内存存储
